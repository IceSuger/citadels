(function() {
  ...
  var connect = function(params, url, cb) {
    ...
    var onopen = function(event) {
      ...
      reset();
      socket =true; //xiaoteng: build.js not have this line! 
      var obj = Package.encode(Package.TYPE_HANDSHAKE, Protocol.strencode(JSON.stringify(handshakeBuffer)));
      send(obj);
	  //processPackage(Package.decode(event.data), cb); //xiaoteng add this line, not aim to do this line, but just do handshake
		//directly do callback!
		if(initCallback) {
		  // initCallback(socket);
		  initCallback();
		}
    ...
    var onmessage = function(event) {
      //processPackage(Package.decode(event.data), cb); //既然onopen里直接执行callback了，这里就先不处理handshake消息了
  ...
  var send = function(packet) {
    //if(socket) //xiaoteng commented this line
